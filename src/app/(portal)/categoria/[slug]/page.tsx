...
export default async function CategoriaPage(props: PageProps) {
  ...
  const [hasCategoriaLateralAds, hasCategoriaFeedAds] = await Promise.all([
    hasActiveAds("categoria", "lateral"),
    hasActiveAds("categoria", "feed"),
  ]);
  const hasCategoriaSidebarAds = hasCategoriaLateralAds || hasCategoriaFeedAds;
  ...
  return (
    <div className="portal-page min-h-[70vh] w-full transition-colors duration-300">
      ...
      <div className="mx-auto max-w-7xl px-4 py-10 sm:px-6 lg:px-8">
        <AdSlot pagina="categoria" posicao="topo" className="mb-10" />
        ...
        {artigos.length === 0 ? (
          ...
          <div className="space-y-10">
            <div className={hasCategoriaSidebarAds ? "grid gap-10 xl:grid-cols-[minmax(0,1fr)_300px] xl:items-start" : ""}>
              <div className={`grid grid-cols-1 gap-10 gap-y-12 md:grid-cols-2 ${hasCategoriaSidebarAds ? "lg:grid-cols-3 xl:grid-cols-2" : "lg:grid-cols-3"}`}>
                {artigos.map((art) => (
              ...
              {hasCategoriaSidebarAds && (
                <div className="hidden space-y-8 xl:block">
                  {hasCategoriaLateralAds && <AdSlot pagina="categoria" posicao="lateral" />}
                  {hasCategoriaFeedAds && <AdSlot pagina="categoria" posicao="feed" />}
                </div>
              )}
            </div>
          </div>
