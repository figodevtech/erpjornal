...
export default async function EpisodePage({ params }: { params: { slug: string } }) {
  ...
  return (
    <article className="max-w-3xl mx-auto px-4 py-12 md:py-20 animate-in fade-in slide-in-from-bottom-6 duration-700">
      <Link href="/podcasts" className="inline-flex items-center gap-2 text-[10px] font-black uppercase tracking-widest text-slate-400 hover:text-red-700 transition-colors mb-4">
      ...
      <div className="space-y-4 mb-12">
        <div className="flex gap-2">
           ...
           <span className="text-[11px] font-black uppercase tracking-widest text-slate-500 bg-slate-100 px-3 py-1 rounded-full">Bastidores</span>
        </div>
        <h1 className="text-4xl md:text-6xl font-black md:tracking-tighter tracking-tight text-slate-900 leading-none">
          {ep.titulo}
        </h1>
      ...
      <div className="mt-12 space-y-8">
        <div className="prose prose-slate max-w-none">
           <h3 className="text-xl font-bold text-slate-900 border-l-[4px] border-red-700 pl-4 mb-4">Sobre este episódio</h3>
           <p className="text-slate-600 text-lg leading-relaxed whitespace-pre-wrap">{ep.descricao}</p>
        </div>

        <div className="pt-8 border-t border-slate-100 flex flex-col md:flex-row items-center justify-between gap-6">
           <div className="flex items-center gap-4">
              <span className="text-xs font-black uppercase tracking-widest text-slate-400">Compartilhar</span>
              <div className="flex items-center gap-3">
                 <button className="w-10 h-10 rounded-full bg-slate-100 flex items-center justify-center hover:bg-slate-200 transition-all text-slate-600">
