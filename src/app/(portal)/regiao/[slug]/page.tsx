...
export default async function RegionalPage({ params }: { params: { slug: string } }) {
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 py-8 space-y-8 animate-in fade-in duration-500">
      {/* Regional Header */}
      <header className="bg-slate-900 text-white rounded-3xl p-10 relative overflow-hidden shadow-2xl">
      ...
      <section className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {articles.map((artigo) => (
          <Link 
            ...
            className="group flex flex-col bg-white border border-slate-200 rounded-2xl overflow-hidden shadow-sm hover:shadow-xl transition-all duration-300 transform hover:-translate-y-1"
          >
            {artigo.og_image_url && (
            ...
            <div className="p-6 flex-1 flex flex-col justify-between space-y-4">
              ...
              <div className="pt-4 border-t border-slate-50 flex items-center justify-between">
                <div className="flex flex-col">
                  <span className="text-[10px] text-slate-400">Por {artigo.autor.nome}</span>
                  <div className="flex items-center gap-1 text-[10px] text-slate-400">
                    <Calendar className="w-3 h-3" />
                    {new Date(artigo.created_at).toLocaleDateString("pt-BR")}
                  </div>
                </div>
                <div className="w-8 h-8 rounded-full bg-slate-100 flex items-center justify-center text-slate-400 group-hover:bg-red-600 group-hover:text-white transition-all">
      ...
      {articles.length === 0 && (
        <div className="py-32 text-center rounded-3xl border-2 border-dashed border-slate-200 bg-slate-50/50">
          ...
          <h3 className="text-xl font-bold text-slate-400">Nenhum artigo encontrado para esta região.</h3>
          <p className="text-slate-400 max-w-sm mx-auto mt-2">Estamos trabalhando para trazer as notícias políticas mais relevantes desta localidade em breve.</p>
        </div>
      )}
    </div>
  );
