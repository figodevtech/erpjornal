            ...
            <div className="bg-slate-50 p-6 rounded-xl border border-slate-200/60 shadow-sm">
                  ...
                  {politicians.map((p) => (
                    <label key={p.id} className="flex items-center gap-3 p-2 hover:bg-slate-50 rounded-lg cursor-pointer transition-colors group">
                      ...
                      <div className="flex flex-col">
                        <span className="text-sm font-medium text-slate-700 group-hover:text-indigo-600 transition-colors">{p.nome}</span>
                        <span className="text-[10px] text-slate-400 font-medium uppercase">{p.partido || "Sem Partido"}</span>
                      </div>
                    </label>
                  ))}
                  {politicians.length === 0 && (
                    <p className="text-xs text-slate-400 italic py-2">Nenhum político cadastrado.</p>
                  )}
                </div>
              </div>
              ...
              <div className="pt-6 border-t border-slate-200/80">
                ...
                <div className="space-y-4">
                  <div>
                    <label className="block text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-1.5">Esfera / Região</label>
                    <select 
                      name="regiao" 
                  ...
                  <div>
                    <label className="block text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-1.5">Estado (Sigla)</label>
                    <input 
                      name="estado" 
                      placeholder="EX: SP"
                      maxLength={2}
              ...
              <div className="pt-6 border-t border-slate-200/80">
                ...
                <div className="space-y-4">
                  <div>
                    <label className="block text-[11px] font-bold text-slate-400 uppercase tracking-wider mb-1.5 font-sans">Status Legal</label>
                    <select 
                      name="legal_status" 
                      defaultValue={initialData?.legal_status || "pendente"}
                      disabled={userRole === "reporter"}
                      className="w-full bg-white border border-slate-200 rounded-lg shadow-sm p-3 text-slate-700 focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 outline-none transition-all text-sm disabled:bg-slate-50 disabled:cursor-not-allowed"
                    >
                  ...
                  <div>
                    <label className="block text-[11px] font-bold text-slate-400 uppercase tracking-wider mb-1.5 font-sans">Notas Jurídicas</label>
                    <textarea 
                      name="legal_notes" 
                      rows={3}
                      defaultValue={initialData?.legal_notes || ""}
                      readOnly={userRole === "reporter"}
                      className="w-full bg-white border border-slate-200 rounded-lg shadow-sm p-3 text-slate-700 focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 outline-none transition-all text-sm resize-none disabled:bg-slate-50 placeholder:italic"
