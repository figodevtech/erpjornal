  ...
  return (
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-black text-gray-900 tracking-tight uppercase italic">Dashboard de Curadoria</h1>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="bg-white border border-gray-200 p-6 rounded-[32px] flex items-center gap-5 shadow-sm">
          <div className="w-12 h-12 bg-gray-100 rounded-2xl flex items-center justify-center">
            <Clock className="w-6 h-6 text-gray-400" />
          </div>
          <div>
            <span className="text-[10px] font-black text-gray-400 uppercase tracking-widest block">Pendentes</span>
      ...
      <div className="flex flex-wrap items-center gap-4 bg-white p-4 rounded-2xl border border-gray-200 shadow-sm">
        ...
        <Link 
          ...
          className={`px-4 py-2 rounded-xl text-xs font-bold transition-all ${!source ? 'bg-gray-900 text-white shadow-lg' : 'bg-gray-50 text-gray-400 hover:bg-gray-100'}`}
        >
          Todos
        </Link>
        {sources.map(s => (
          <Link 
            ...
            className={`px-4 py-2 rounded-xl text-xs font-bold transition-all ${source === s.id ? 'bg-gray-900 text-white shadow-lg' : 'bg-gray-50 text-gray-400 hover:bg-gray-100'}`}
          >
            {s.name}
          </Link>
        ))}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 2xl:grid-cols-3 gap-6">
        ...
        {items.length === 0 && (
          <div className="md:col-span-2 2xl:col-span-3 py-32 text-center rounded-[40px] border-2 border-dashed border-gray-200 bg-white/50">
            <X className="w-16 h-16 text-gray-200 mx-auto mb-4" />
            <h3 className="text-xl font-black text-gray-900 uppercase">Sem notícias pendentes</h3>
      ...
      <div className="pt-12 border-t border-gray-200">
         <h2 className="text-xl font-black text-gray-900 uppercase tracking-tight mb-6">Recentemente Publicados</h2>
                  ...
                  {(await prisma.article.findMany({
                     ...
                     <tr key={art.id} className="hover:bg-gray-50/50 transition-colors">
                        <td className="px-6 py-4">
                           <p className="text-sm font-bold text-gray-900 leading-tight line-clamp-1">{art.titulo}</p>
                           <p className="text-[10px] text-indigo-500 font-black uppercase tracking-widest mt-0.5">{art.categoria?.nome}</p>
                        </td>
                        <td className="px-6 py-4 text-xs font-medium text-gray-500">{art.external_author}</td>
                        <td className="px-6 py-4 text-xs font-medium text-gray-500 italic">{art.human_reviewer}</td>
                        <td className="px-6 py-4 text-xs font-medium text-gray-400">{new Date(art.created_at).toLocaleDateString()}</td>
