  ...
  if (session.user.role !== "admin") {
    return (
      <div className="flex flex-col items-center justify-center py-20 text-center">
        ...
        <h1 className="text-2xl font-black text-slate-900 uppercase tracking-widest">Acesso Restrito</h1>
  ...
  return (
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-black text-slate-900 tracking-tight uppercase">Fontes RSS</h1>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        {fontes.map((fonte) => (
          <div 
            key={fonte.id} 
            className="bg-white border boundary-red border-slate-200 rounded-[28px] p-6 hover:shadow-xl transition-all group overflow-hidden relative shadow-sm"
          >
            ...
            <div className="relative z-10">
              <div className="flex items-start justify-between mb-6">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <h3 className="text-xl font-black text-slate-900 uppercase tracking-tight">{fonte.name}</h3>
              ...
              <div className="grid grid-cols-2 gap-4 mb-6">
                <div className="bg-slate-50 p-3 rounded-2xl">
                  <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest block mb-1">Itens Coletados</span>
                  <span className="text-2xl font-black text-slate-900">{fonte._count.items}</span>
                </div>
                <div className="bg-slate-50 p-3 rounded-2xl">
                  <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest block mb-1">Status TTL</span>
                  <span className="text-sm font-bold text-slate-600">{fonte.cache_ttl} min</span>
                </div>
              </div>
              ...
              <p className="mt-4 text-[11px] font-mono text-slate-400 break-all bg-slate-50 p-2 rounded-lg border border-slate-100 italic">
                {fonte.feed_url}
              </p>
            </div>
          </div>
        ...
        {fontes.length === 0 && (
          <div className="lg:col-span-2 py-32 text-center rounded-[40px] border-2 border-dashed border-slate-200 bg-white/50">
            <Globe className="w-16 h-16 text-slate-200 mx-auto mb-4" />
            <h3 className="text-xl font-black text-slate-900 uppercase">Nenhum feed RSS</h3>
