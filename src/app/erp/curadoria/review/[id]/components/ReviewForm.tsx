...
export function ReviewForm({ item, aiData, user }: { item: any; aiData: any; user: any }) {
  ...
  return (
    <form onSubmit={handlePublish} className="space-y-8 animate-in fade-in duration-1000">
      ...
      {/* Botão Gatilho IA */}
      <div className="flex flex-col md:flex-row items-center justify-between p-6 bg-slate-900 rounded-[32px] text-white shadow-xl shadow-indigo-500/10 relative overflow-hidden group border border-white/10">
        ...
        <button
          ...
          onClick={handleAI}
          disabled={loadingIA}
          className="mt-6 md:mt-0 bg-white text-slate-900 font-black uppercase tracking-widest px-8 py-5 rounded-2xl hover:bg-indigo-500 hover:text-white transition-all flex items-center gap-3 shadow-lg active:scale-95 disabled:opacity-50 text-xs relative z-10"
        >
      ...
      <div className="bg-white border border-slate-200 rounded-[40px] p-6 md:p-10 shadow-sm space-y-8">
        
        {/* Título Final */}
        <div className="space-y-2">
          <label className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] ml-2">Título Redesign</label>
          <input 
            value={titulo}
            onChange={(e) => setTitulo(e.target.value)}
            className="w-full bg-slate-50 border-2 border-slate-100 rounded-3xl p-6 text-2xl font-black text-slate-900 focus:bg-white focus:border-indigo-500 outline-none transition-all placeholder:text-slate-300"
        ...
        {/* Resumo/Lead */}
        <div className="space-y-2">
          <label className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] ml-2">Lead (Resumo)</label>
          <textarea 
            rows={3}
            value={resumo}
            onChange={(e) => setResumo(e.target.value)}
            className="w-full bg-slate-50 border-2 border-slate-100 rounded-3xl p-6 text-sm font-bold text-slate-600 focus:bg-white focus:border-indigo-500 outline-none transition-all resize-none italic"
            placeholder="Breve resumo atraente..."
          />
        </div>

        <div className="space-y-2">
          <label className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] ml-2">Corpo da Matéria</label>
          <RichTextEditor 
            content={corpo} 
            onChange={setCorpo} 
          />
        ...
        <div className="pt-8 border-t border-slate-100 flex flex-col md:flex-row items-center justify-between gap-6">
          <div className="flex items-center gap-4">
             ...
             <div>
                <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Revisor Humano</p>
                <p className="text-sm font-black text-slate-900 uppercase">Portal &bull; {user.name || 'Editor'}</p>
             </div>
          </div>
          ...
          <div className="flex items-center gap-3 shrink-0 w-full md:w-auto">
             <button
                ...
                className="flex-1 md:flex-none flex items-center justify-center gap-2 px-6 py-4 rounded-2xl border border-slate-200 text-slate-400 font-black uppercase tracking-widest text-[10px] hover:bg-rose-50 hover:text-rose-500 hover:border-rose-100 transition-all"
             >
      ...
      <div className="p-8 bg-slate-900 rounded-[40px] text-white/50 border border-white/10 italic text-xs space-y-1 relative overflow-hidden">
        ...
        <p>🔗 Baseado em matéria original do portal <strong>{item.source.name}</strong></p>
        <p>📅 Publicado originalmente em {new Date(item.pub_date).toLocaleDateString('pt-BR')} às {new Date(item.pub_date).toLocaleTimeString('pt-BR', {hour:'2-digit', minute:'2-digit'})}</p>
        <p>✍️ Conteúdo reescrito via IA e revisado por <strong>{user.name || 'Redação'}</strong></p>
