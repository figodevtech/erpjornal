...
async function buscarSessaoAtualizada() {
  const response = await fetch("/api/auth/me", { cache: "no-store" });
  if (!response.ok) return null;
  const data = await response.json();
  return data.sessao as AuthPortalProps["session"];
}
  ...
  async function handleLogin(e: React.FormEvent) {
    ...
    try {
      ...
      if (signInError) {
        ...
        setError(traduzirErroLoginSupabase(signInError));
        return;
      }
          ...
          {user ? (
            ...
            <div className="p-6">
              <div className="mb-6 text-center">
                ...
                <p className="text-[12px] font-bold text-gray-500 dark:text-gray-400 uppercase tracking-widest mt-1">
                  Acesso Exclusivo
                </p>
              </div>
              ...
              <form onSubmit={handleLogin} className="space-y-4">
                ...
                <div className="space-y-1.5">
                  <label htmlFor="login-email" className="text-[11px] font-black text-gray-500 dark:text-gray-400 uppercase tracking-widest ml-1">
                    E-mail
                  </label>
                ...
                <div className="space-y-1.5">
                  <div className="flex justify-between items-center ml-1">
                    <label htmlFor="login-password" className="text-[11px] font-black text-gray-500 dark:text-gray-400 uppercase tracking-widest">
                      Senha
                    </label>
                    <Link href="/recuperar" className="text-[10px] font-bold text-red-600 hover:text-red-700 transition-colors uppercase">
                      Esqueci
                    </Link>
                  </div>
                  <div className="relative">
                ...
                >
                  {isLoading ? <Loader2 className="w-4 h-4 animate-spin" /> : "ENTRAR AGORA"}
                </button>
              </form>

              <div className="mt-6 pt-6 border-t border-gray-100 dark:border-gray-800 text-center">
                <p className="text-xs text-gray-500 dark:text-gray-400 font-medium mb-2">Não tem uma conta?</p>
                <Link href="/assine" className="text-xs font-black text-red-700 hover:text-red-800 uppercase tracking-tight">
                  Assinar Revista Gestão
                </Link>
              </div>
            </div>
          )}
