...
async function test() {
  try {
    ...
    const author = await prisma.usuario.findFirst({ where: { artigosAutor: { some: {} } } });
    if (author) {
      ...
      const directCount = await prisma.artigo.count({ where: { autorId: author.id, status: "publicado" } });
